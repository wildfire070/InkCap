...
// bookshelf (BookFusion-only, no BookFusion here) or publisher/tags/star-rating
// fields. Sibling prev/next book navigation (Left/Right, matching InsiderPhD)
// is supported, but this Activity owns none of the list logic for it -- the
// caller passes whether a previous/next book exists, and Left/Right just
// finish() with a BookDetailsNavResult so the caller (which already has the
// real book list -- see each caller's own openBookDetails()-style helper) can
// re-launch this Activity for that book.
