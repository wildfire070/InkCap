...
struct HomeMenuEntries {
  ...
  // optional entry appendHomeMenuItems() can add (OPDS, AO3, BookFusion, Reading
  ...
  if (isCarousel) {
    ...
    auto handleTouch = [&](const bool activate) {
      ...
      if (activate && mappedInput.wasItemTapped(touchedMenuIndex)) {
        if (touchedMenuIndex < 0 || touchedMenuIndex >= menuItemCount) return false;
        carouselMenuTouchDownIndex = -1;
        const auto menuItems = buildHomeMenuItems(hasOpdsServers, hasAo3Library, hasBookFusion, hasReadingStats,
...
void HomeActivity::render(RenderLock&&) {
  ...
  GUI.drawButtonMenu(renderer, menuRect, static_cast<int>(menuItems.size()), menuSelectedIndex, labelCStrAt,
                     [&menuItems](int index) { return menuItems[index].icon; });
