...
void drawMissingBookCover(const GfxRenderer& renderer, const Rect& coverRect, const RecentBook& book) {
  ...
  drawLucideIcon(renderer, blankCoverIconFor(book.path), coverRect.x + (coverRect.width - iconSize) / 2, coverRect.y + 36);
  ...
  if (!coverBmpPath.empty() && Storage.exists(coverBmpPath.c_str())) {
    ...
    if (Storage.openFileForRead("HOME", coverBmpPath, file)) {
      ...
      if (bitmap.parseHeaders() == BmpReaderError::Ok) {
        ...
        // divide by 8 -- non-aligned values shift the icon relative to the white fill.
        // This theme has no InsiderPhD equivalent to port from; extended here to match
        // Lyra/RoundedRaff/Lyra3Covers, which do.
