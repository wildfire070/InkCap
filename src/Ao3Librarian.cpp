...
class HtmlScraper : public Print {
 public:
  ...
  size_t write(uint8_t b) override {
    ...
    if (inSummary) {
      ...
      if (c == '>') {
        ...
        if (bufferSize > 13 && (strstr(buffer + bufferSize - 13, "</div>") ||
                                 strstr(buffer + bufferSize - 13, "</span>") || strstr(buffer + bufferSize - 13, "</blockquote>"))) {
  ...
  void processBuffer() {
    ...
    if (scrapedRel1[0] == '\0' && scrapedRel2[0] == '\0') {
        ...
        if (relAnchor) {
            ...
            if (splitMainAdditional && scrapedRel2[0] == '\0' &&
                strstr(buffer, "Additional Relationships:")) {
  ...
  void tryExtractFFFTitlePageSummary() {
    ...
    while (scan < buffer + bufferSize && summaryBytes < 511) {
      if (*scan == '<') {
        if (static_cast<size_t>(buffer + bufferSize - scan) >= 6 &&
            strncmp(scan, "</div>", 6) == 0) {
          closed = true;
          break;
        }
        inTag = true;
...
uint32_t Ao3Librarian::estimateWordCount(const Epub& epub) {
  ...
  static const char* skipMarkers[] = {
    "cover", "title_page", "titlepage", "log_page", "logpage", "toc", "nav", "copyright"
  };
  ...
  for (int i = 0; i < spineCount; i++) {
    ...
    for (const char* marker : skipMarkers) {
      if (lowerHref.find(marker) != std::string::npos) { skip = true; break; }
    }
    if (skip) continue;
...
bool Ao3Librarian::scrape(const Epub& epub, bool force) {
  ...
  if (!force && Storage.exists(infoPath.c_str())) {
    ...
    if (getLibraryInfo(epub, *existing)) {
      if (existing->version == 9 && existing->rating != '-' && existing->warning != 0 && existing->summary[0] != 0 && existing->wordCount > 0) {
