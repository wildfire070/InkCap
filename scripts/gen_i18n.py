...
def generate_keys_header(
    ...
    # A V1 code with no corresponding translation currently built in (e.g. a
    # language dropped to save flash) falls back to EN rather than failing to
    # compile, so a build can carry any subset of languages.
    v1_codes = [
        "EN", "ES", "FR", "DE", "CS", "PT", "RU", "SV", "RO", "CA", "UK",
        "BE", "IT", "PL", "FI", "DA", "NL", "TR", "KK", "HU", "LT", "SI",
    ]
    v1_exprs = [f"Language::{c}" if c in languages else "Language::EN" for c in v1_codes]
    lines.append("// V1 language.bin migration table (frozen enum order from 2f969a9)")
    lines.append("constexpr Language V1_LANGUAGES[] = {")
    lines.append("    " + ", ".join(v1_exprs) + ",")
    lines.append("};")
    lines.append(
...
def main(
    ...
    try:
        ...
        if builtin is None:
            ...
            compiled_codes = ", ".join(c for c in languages if c in builtin)
            print(f"  Languages: {len(builtin)} of {len(languages)} built in ({compiled_codes})")
