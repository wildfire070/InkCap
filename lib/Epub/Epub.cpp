...
bool Epub::load(const bool buildIfMissing, const bool skipLoadingCss, const XLocationLoadMode xLocationLoadMode,
                const bool cacheCumulativeSpineSizes, const bool skipScraping) {
  ...
  if (bookMetadataCache->load()) {
    ...
    if (!hasAo3Info() && !bookMetadataCache->coreMetadata.ao3WorkId.empty()) {
      saveAo3Info(bookMetadataCache->coreMetadata.ao3WorkId, bookMetadataCache->coreMetadata.ao3UpdateDate,
                  bookMetadataCache->coreMetadata.ao3IsCompleted);
    }

    // Global Library: Generate/Fix extended metadata sidecar (FanFicFare ao3WorkId or AO3-download preface)
    if (!skipScraping) {
...
bool Epub::hasAo3Info() const { return Storage.exists((cachePath + "/ao3-info.bin").c_str()); }

std::string Epub::getAo3WorkId() const {
  HalFile f;
  if (Storage.openFileForRead("EBP", cachePath + "/ao3-info.bin", f)) {
    if (f.size() < 9) {
      f.close();
...
bool Epub::isAo3Completed() const {
  ...
  if (Storage.openFileForRead("EBP", cachePath + "/ao3-info.bin", f)) {
    if (f.size() < 9) {
      f.close();
      return bookMetadataCache ? bookMetadataCache->coreMetadata.ao3IsCompleted : false;
    }
    bool completed;
    serialization::readPod(f, completed);
    f.close();
...
void Epub::saveAo3Info(const std::string& workId, const std::string& date, const bool completed) const {
  HalFile f;
  if (Storage.openFileForWrite("EBP", cachePath + "/ao3-info.bin", f)) {
    serialization::writePod(f, completed);
    serialization::writeString(f, workId);
...
class PublisherParser final : public Print {
  ...
  static void characterData(void* userData, const XML_Char* s, int len) {
    ...
    if (self->state == IN_DC_PUBLISHER && self->publisher.size() < kMaxRawBytes) {
      ...
      self->publisher.append(s, toCopy);
      if (toCopy < static_cast<size_t>(len)) {
        // Truncated mid-chunk -- back up to the last complete UTF-8 codepoint.
        self->publisher.resize(utf8SafeTruncateBuffer(self->publisher.data(), static_cast<int>(self->publisher.size())));
      }
    }
  }
...
std::string Epub::sniffPublisher() const {
  ...
  // call and getDescription()'s below (see that one's comment for the full
  // story: with opfSize as the chunk size, a field far enough into a large
  // content.opf -- e.g. dc:publisher after a BookFusion custom-column metadata
  // blob -- came back silently empty).
...
std::string Epub::getDescription() const {
  ...
  // 1024, not opfSize -- matches parseContentOpf()'s own call below, and
  // ContentOpfParser's write() one further down still, both of which stream
  // content.opf through a small fixed-size chunk rather than one buffer sized
  // to the whole (often tens-of-KB) file. Confirmed on-device: with opfSize as
  // the chunk size here, dc:description silently came back empty on every book
  // whose <dc:description> sits far enough into the file (BookFusion-linked
  // EPUBs put a large custom-column meta block before it, past ~32KB) --
  // whatever the exact mechanism, the small fixed chunk is the pattern proven
  // to parse this same file correctly elsewhere (title/author/etc. always load
  // fine even on those books), so this follows it instead of re-deriving it.
  if (!readItemContentsToStream(opfPath, parser, 1024)) {
    return "";
  }
