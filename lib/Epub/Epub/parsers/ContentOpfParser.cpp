...
void XMLCALL ContentOpfParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (self->state == IN_METADATA && strcmp(name, "dc:creator") == 0) {
    ...
    // The actual ", " is appended lazily by appendMetadataText() (via
    // authorSeparatorPending below), once real text for this creator
    // arrives -- appending it here too double-separated multi-author books.
  ...
  if (self->state == IN_METADATA && (strcmp(name, "meta") == 0 || strcmp(name, "opf:meta") == 0)) {
    ...
    } else if (nameAttr && contentAttr && strcmp(nameAttr, "calibre:user_metadata:#chapters") == 0) {
      // FanFicFare's posted/total chapter count, e.g. "1/1" or "3/7".
      self->chapters = extractCalibreCustomValue(contentAttr);
    } else if (nameAttr && contentAttr && strcmp(nameAttr, "calibre:user_metadata:#completionstatus") == 0) {
      self->completionStatus = extractCalibreCustomValue(contentAttr);
    } else if (nameAttr && contentAttr && strcmp(nameAttr, "calibre:user_metadata:#updated") == 0) {
      // The story's own last-update date on AO3 -- distinct from calibre:timestamp
      // (the Calibre/FanFicFare *export* timestamp, not tracked in this branch).
    ...
    } else if (nameAttr && contentAttr && strcmp(nameAttr, "calibre:user_metadata:#like") == 0) {
