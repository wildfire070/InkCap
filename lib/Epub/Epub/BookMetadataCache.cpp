...
constexpr uint8_t BOOK_CACHE_VERSION = 12;  // v12: added tags (dc:subject); v11: added seriesName,
                                            // seriesIndex, contentRating, chapters, completionStatus,
                                            // updatedDate, liked, readStatus metadata
...
bool BookMetadataCache::buildBookBin(const std::string& epubPath, const BookMetadata& metadata) {
  ...
  const uint32_t metadataSize = metadata.title.size() + metadata.author.size() + metadata.language.size() +
                                ...
                                metadata.ao3UpdateDate.size() + metadata.seriesName.size() +
